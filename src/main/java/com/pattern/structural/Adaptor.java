 ...
 *  3. 만일 추가로 필요한 메소드가 있다면 어댑터에 빠르게 만들 수 있음. 만약 버그가 발생해도 기존의 클래스에는 버그가 없기 때문에 Adaptor 역할의 클래스를 중점적으로 조사하면 되고 프로그램 검사도 쉬워짐
 * 단점 ->
 ...
 * 2. Target(Client Interface) : Adaptor 가 구현하는 인터페이스
 * 3. Adaptor : Client 와 Adaptee(Service) 중간에서 호환성이 없는 둘을 연결시켜주는 역할을 담당
